...
# RPi Router (USB Tethering) Installer
# This script configures the Raspberry Pi to act as a router,
# sharing an internet connection from a USB-tethered phone (usb0)
...
echo "🚀 Starting Raspberry Pi USB Router Setup..."

# 1. Install necessary packages
echo "📦 Installing dnsmasq and iptables-persistent..."
sudo apt-get update
sudo apt-get install -y dnsmasq iptables-persistent net-tools

# 2. Configure a static IP for the LAN interface
echo "⚙️ Configuring static IP ${LAN_IP}/${LAN_SUBNET} for ${LAN_INTERFACE}..."
# Check if a connection with the same name already exists and delete it
if nmcli con show "$NMCLI_CONNECTION_NAME" > /dev/null 2>&1;
then
...
echo "📝 Configuring dnsmasq for DHCP on ${LAN_INTERFACE}..."
...
echo "🔥 Setting up firewall rules (NAT and security)..."
...
echo "🛡️ Applying security hardening rules to ${WAN_INTERFACE}..."
# Drop common port scans
sudo iptables -A INPUT -i "$WAN_INTERFACE" -p tcp --dport 23 -j DROP   # Telnet
sudo iptables -A INPUT -i "$WAN_INTERFACE" -p tcp --dport 445 -j DROP  # SMB
sudo iptables -A INPUT -i "$WAN_INTERFACE" -p tcp --dport 135 -j DROP  # RPC

# Rate-limit new connections to prevent simple floods
sudo iptables -A INPUT -i "$WAN_INTERFACE" -p tcp --syn -m limit --limit 1/second --limit-burst 3 -j ACCEPT
sudo iptables -A INPUT -i "$WAN_INTERFACE" -p tcp --syn -j DROP
...
echo ""
echo "✅ Raspberry Pi USB Router setup is complete!"
